...
exports.run = async (client, message, args) => {
  ...
  message.channel.send(`Deleted ${client.users.get(alıcı) ? client.users.get(alıcı).tag : alıcı} ${para} Coins${reason}.`)
  let array = db.fetch(`transactions_${alıcı}`) || []
  array.unshift(`%${Date.now()}% - Admin deleted your ${para} coins${reason}. (-${para})`)
  db.set(`transactions_${alıcı}`, array)
  let arraya = db.fetch(`transactions_${message.author.id}`) || []
  arraya.unshift(`%${Date.now()}% - You deleted ${para} coins of ${client.users.get(alıcı) ? client.users.get(alıcı).tag : alıcı}${reason}. (0)`)
...
exports.conf = {
    enabled: true,
    guildOnly: false,
    aliases: ['paraeksilt', 'delete', 'eksilt', 'deletecoins', 'sil'],
    permLevel: 4,
    katagori: "Ekonomi"
}
exports.help = {
